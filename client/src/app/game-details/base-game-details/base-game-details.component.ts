...
@Component({
  ...
  templateUrl: './base-game-details.component.html',
  styleUrl: './base-game-details.component.css'
})
export class BaseGameDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.fetchLeagues();
    this.team1FilteredOptions = this.teamsFormGroup.get('team1')?.valueChanges.pipe(
      startWith(''),
      map((value) => this._filterTeam1(value || '')),
    );
    this.team2FilteredOptions = this.teamsFormGroup.get('team2')?.valueChanges.pipe(
      startWith(''),
  ...
  get team1Goals(): number {
    return this.team1PlayerStats.reduce((total, stat) => total + (stat.goals_scored || 0), 0);
  }

  get team2Goals(): number {
    return this.team2PlayerStats.reduce((total, stat) => total + (stat.goals_scored || 0), 0);
  }
  ...
  fetchTeamPlayersForSeason(team1Name: string, team2Name: string): void {
    ...
    const season = this.leagueAndSeasonFormGroup.get('season')?.value;
    if (season) {
      const team1PlayersRequest = this.teamService.getTeamPlayers(team1Name, season);
      const team2PlayersRequest = this.teamService.getTeamPlayers(team2Name, season);
      const date = this.selectedGame.match_date;
      const formattedDate = new Date(date).toISOString().split('T')[0];
      console.log(formattedDate);
      const playerStatRequest = this.playerStatService.getPlayerStats(team1Name, team2Name, formattedDate)

      // Using forkJoin to combine both requests
      forkJoin([team1PlayersRequest, team2PlayersRequest, playerStatRequest]).subscribe({
        next: ([team1Players, team2Players, psResp]: [Player[], Player[], PlayerStat[]]) => {
          ...
          this.prepareFilledPlayerStats(team1Players, team2Players, team1Name, team2Name, this.selectedGame.match_date);
        },
        error: (err) => {
  ...
  prepareFilledPlayerStats(team1Players: Player[], team2Players: Player[], team1Name: string, team2Name: string, matchDate: string) {
    // Prepare stats for Team 1 players
    this.team1PlayerStats = team1Players.map(player => {
      const stat = this.selectedGamePlayerStats.find(stat => stat.player_id === player.player_id);
      return {
  ...
  createPlayerStatPayload(): any {
    ...
    const nonZeroPlayerStats = [
      ...
      ...this.team2PlayerStats.filter(stat => (stat.goals_scored + stat.yellow_card + stat.red_card) != 0)
    ];
    const date = this.selectedGame.match_date;
    const formattedDate = new Date(date).toISOString().split('T')[0];
