...
export class HomePageComponent implements OnInit {
  ...
  fetchSponsorDetails() {
    const requests = this.leagueNames.map(el =>
      this.sponsorService.getSponsorshipDetailByLeague(el) // Return the observable
    );

    forkJoin(requests).subscribe({
      next: (responses: SponsorLeague[][]) => { // Adjust type to match response shape
    ...
    const datasets = sponsorNames.map(((sponsorName, index) => {
      return {
        ...
        data: this.leagueSponsorDetails.map(leagueSponsors => {
          const sponsor = leagueSponsors.find(s => s.sponsor_name === sponsorName);
...
// Helper function to generate random colors
  ...
  fetchLeagueStats() {
    ...
    const observables = this.leagueNames.flatMap(leagueName =>
      this.seasons.map(season => this.leagueService.getLeagueStats(leagueName, season))
    );
  ...
  setHomePageChartData() {
    ...
    this.leagueNames.forEach(leagueName => {
      ...
      const teamStatsMap = new Map<string, { season: number; rank: number }[]>();

      statsForLeague.forEach(stat => {
        if (!teamStatsMap.has(stat.team_name)) {
          teamStatsMap.set(stat.team_name, []);
        }
        teamStatsMap.get(stat.team_name)!.push({ season: stat.season, rank: stat.team_rank || 0 });
      });
