...
export class PlayerStatService {
  ...
  getPlayerStats(team1Name: string, team2Name: string, matchDate: string) {
    ...
    return this.http.get<PlayerStat[]>(`${this.playerStatsBaseUrl}/get`, { params });
  }


  insertPlayerStat(payload: any): Observable<any> {
    return this.http.post<any>(`${this.playerStatsBaseUrl}/insert`, payload)
      .pipe(
        catchError((error) => {
  ...
  fetchTopStat(leagueName: string, season: number, statName: string): Observable<{top_stats: TopStatRow[]}> {
    const params = new HttpParams()
      .set('league_name', leagueName)
      .set('season', season.toString())
      .set('stat_name', statName);

    return this.http.get<any>(`${this.playerStatsBaseUrl}/top_stat`, { params });
  }

  fetchPlayerStatsForSeasons(player_id: number, seasonList: string): Observable<PlayerStatRow[]> {
  const params = new HttpParams()
    .set('player_id', player_id)
    .set('season', seasonList.toString())

  return this.http.get<PlayerStatRow[]>(`${this.playerStatsBaseUrl}/for_seasons`, { params });
  }
